# 🌦️ Weather App

<p align="center">

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Responsive](https://img.shields.io/badge/Responsive-Yes-success?style=for-the-badge)
![MIT License](https://img.shields.io/badge/License-MIT-blue?style=for-the-badge)

</p>

A modern and responsive **Weather App** built using **HTML5, CSS3, and Vanilla JavaScript**. It provides real-time weather information with an elegant glassmorphism interface, animated weather icons, dynamic themes, and a smooth user experience across all devices.

---

# ✨ Features

### 🌍 Weather Information

- 🌡️ Real-time weather conditions
- 🤗 Feels Like temperature
- 💧 Humidity
- 🌬️ Wind Speed
- 👀 Visibility
- 📈 Atmospheric Pressure
- 🌅 Sunrise & Sunset
- ☁️ Weather Description

### 📅 Forecasts

- ⏰ Hourly Forecast
- 📆 7-Day Forecast
- 💡 Smart Weather Insights
- 📊 Comfort Score

### 🔍 Search

- Search weather by city
- Smart search suggestions
- Recent searches
- Favourite cities

### 🎨 User Experience

- Dynamic weather-based themes
- Premium Glassmorphism UI
- Animated SVG weather icons
- Smooth animations and transitions
- Skeleton loading screens
- Beautiful error handling
- Online/Offline status detection
- Fully responsive layout
- Accessibility-friendly interface

### ⚡ Performance

- Search caching
- Debounced API requests
- Local Storage support
- Fast and optimized rendering

---

# 🛠️ Built With

- HTML5
- CSS3
- Vanilla JavaScript (ES6)
- OpenWeatherMap API
- Geolocation API
- Local Storage API

---

# 🎨 UI Highlights

- Modern Glassmorphism Design
- Dynamic Weather Themes
- Animated Mesh Background
- Animated SVG Weather Icons
- Comfort Score Progress Ring
- Interactive Forecast Cards
- Expandable Panels
- Smooth Micro-interactions

---

# 📂 Project Structure

```text
Weather-App/
│
├── index.html
├── README.md
└── screenshots
    
```

---

# 🚀 Getting Started

## 1. Clone the Repository

```bash
git clone https://github.com/yourusername/weather-app.git
```

## 2. Navigate to the Project

```bash
cd weather-app
```

## 3. Get an API Key

Create a free API key from:

https://openweathermap.org/api

Replace

```javascript
const API_KEY = "YOUR_API_KEY";
```

with your own API key.

## 4. Run the Application

Open

```text
index.html
```

in your favourite web browser.

---

# 📱 Responsive Design

The application is fully responsive and optimized for:

- 💻 Desktop
- 💼 Laptop
- 📱 Mobile
- 📲 Tablet

---

# 🚀 Future Improvements

- Progressive Web App (PWA)
- Air Quality Index (AQI)
- UV Index
- Weather Radar
- Interactive Maps
- Weather Alerts
- Multi-language Support
- Dark & Light Mode
- Hourly Weather Charts

---

# 🤝 Contributing

Contributions are welcome!

1. Fork the repository.

2. Create a new branch.

```bash
git checkout -b feature-name
```

3. Commit your changes.

```bash
git commit -m "Add new feature"
```

4. Push the branch.

```bash
git push origin feature-name
```

5. Open a Pull Request.

---

# 👨‍💻 Author

**Muhammad Furqan**

Computer Science Student

**GitHub:**  
https://github.com/yourusername

**LinkedIn:**  
https://www.linkedin.com/in/muhammad-furqan-228807304/d 

---

# ⭐ Support

If you found this project useful, consider giving it a **⭐ Star** on GitHub. It helps others discover the project and motivates further improvements.

---

# 📄 License

This project is licensed under the **MIT License**.
